...
var _ rpc.OrderWorkerRPC = (*orderWorkerRPC)(nil)

type orderWorkerRPC struct {
	ctx  context.Context
	host string
	port string
}

func NewOrderWorkerRPC(ctx context.Context, host, port string) rpc.OrderWorkerRPC {
	return orderWorkerRPC{ctx: ctx, host: host, port: port}
}

func (p orderWorkerRPC) SaveOrder(order dto.RequestOrder) (*dto.OutputOrder, error) {
...
func (p orderWorkerRPC) UpdateOrderByID(id string, order dto.RequestOrder) (*dto.OutputOrder, error) {
	conn, err := grpc.NewClient(fmt.Sprintf("%s:%s", p.host, p.port), grpc.WithTransportCredentials(insecure.NewCredentials()))
...
func (p orderWorkerRPC) GetOrderByID(id string) ([]dto.OutputOrder, error) {
	conn, err := grpc.NewClient(fmt.Sprintf("%s:%s", p.host, p.port), grpc.WithTransportCredentials(insecure.NewCredentials()))
...
func (p orderWorkerRPC) GetOrders() (*dto.OutputOrder, error) {
	conn, err := grpc.NewClient(fmt.Sprintf("%s:%s", p.host, p.port), grpc.WithTransportCredentials(insecure.NewCredentials()))
